...
export function* planCommand(flags: PlanFlags): Operation<void> {
  if (flags.verbose) {
    yield* log.debug("Running plan command with flags:", flags);
  }

  yield* log.info("Planning publication...");

  if (flags.extName) {
    yield* log.info(`Planning for extension: ${flags.extName}`);
  } else {
    yield* log.info("Planning for all extensions");
  }

  if (flags.effection) {
    yield* log.info(`Planning for Effection version: ${flags.effection}`);
  }
  ...
  yield* log.info(`Planning for registries: ${registries.join(", ")}`);
  ...
  yield* log.info("Planning complete");
}
