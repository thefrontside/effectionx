...
# Usage: ./test-v4-node.sh [test-pattern] [iterations]
# Examples:
#   ./test-v4-node.sh                          # Run all tests once
#   ./test-v4-node.sh 'watch/**/*.test.ts'     # Run watch tests once
#   ./test-v4-node.sh 'watch/**/*.test.ts' 10  # Run watch tests 10 times
#   ./test-v4-node.sh '' 50                    # Run all tests 50 times

test_pattern=${1:-./**/*.test.ts}
total=${2:-1}
failures=0
timestamp=$(date +%Y%m%d_%H%M%S)
summary_file="test-summary-v4_${timestamp}.log"

echo "V4 Node Test Run - Started at $(date)" > "$summary_file"
...
echo "==========================================" >> "$summary_file"
echo "V4 Tests - Total failures: $failures/$total" >> "$summary_file"
