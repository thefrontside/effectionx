...
# Pane 2: Pre-type a kill command that kills the cook's node process (not the shell)
tmux send-keys -t "${SESSION}:0.2" "kill -9 \$(pgrep -P \$(tmux display-message -p -t \"${SESSION}:0.1\" '#{pane_pid}'))"
